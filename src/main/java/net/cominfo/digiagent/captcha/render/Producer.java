...
import net.cominfo.digiagent.captcha.impl.DefaultBackgroundImpl;
...
public final class Producer {
	...
	public static IWordFactory forName(String className) throws Exception {
		...
		if (inst == null) {
			throw new ClassCastException("Passed In class " + className
					+ " does not implement getInstance() method");
		}

		Object o = inst.invoke(null, new Object[] {});
		if (!(o instanceof IWordFactory)) {
			throw new ClassCastException("Passed In class " + className
					+ " is not an instance of  org.smx.captcha.IWordFactory");
		}
	 ...
	 * @throws Exception
	 */
	public static boolean render(OutputStream os, IWordFactory inst,
			Properties props) throws Exception {
		FORCE_ALPHA_CHANNEL = false;

		String format = props.getProperty("format", "jpg").toLowerCase();
		if (!format.equals("jpg") && !format.equals("png")) {// Invalid JPG |
																// PNG Format
	...
	private static BufferedImage renderImage(IWordFactory inst, Properties props) {
		...
		/*
		 * Default Properties format - PNG, JPG default JPG font - Java Font
		 * Name default "Helvetica" fontsize - defaults to 20 min-height :
		 * mininum height of the image defaults to dynamic size -1 min-width :
		 * minimun width of the image defaults to dynamic size -1 padding-x : x
		 * padding default 20 padding-y : Defaults dynamicly to size of text/2
		 */
		...
		String fontName = props
				.getProperty("font", Constants.DEFAULT_FONT_NAME);
		fontSize = Integer.valueOf(props.getProperty("fontsize",
				Constants.DEFAULT_FONT_SIZE));
		min_width = Integer.valueOf(props.getProperty("min-width",
				Constants.DEFAULT_IMGAE_WIDTH));
		// min_height=Integer.valueOf(props.getProperty("min-height",
		// Constants.DEFAULT_IMGAE_HEIGHT));
		...
		BufferedImage buffer = gc.createCompatibleImage(1, 1,
				IMAGE_TYPE_BUFFERED);
		...
		int padding_x = Integer.valueOf(props.getProperty("padding-x",
				Constants.DEFAULT_IMGAE_PADDING_X));
		int padding_y = Integer.valueOf(props.getProperty("padding-y",
				(yBounds / 2) + ""));
		boolean isBezierCurve = Boolean.valueOf(props.getProperty("curve",
				"true"));
		// padding_y=30;
		...
		if (BackGroundProducer instanceof BackgroundImageAssembler
				&& props.getProperty("format", "jpg").equals("jpg")) {
		...
		for (int k = 0; k < curve.length; k++) {
			...
			ystart = Math.round(curve[k].y) - (int) letterBounds.getCenterY()
					+ startYPosition;
		...
		if (isBezierCurve) {
			...
			for (int k = 0; k < curve.length; k++) {
				if (k % 10 == 0) {
					letterBounds = font.getStringBounds(
							"" + text.charAt(index), fc);
	 ...
	 * points generated from the control points cp. Caller must allocate
	 * sufficient memory for the result, which is <sizeof(BezierPoint2D)
	 * numberOfPoints> void ComputeBezier( BezierPoint2D* cp, int
	 * numberOfPoints, BezierPoint2D* curve ) { float dt; int i;
	 * 
	 * dt = 1.0 / ( numberOfPoints - 1 );
	 * 
	 * for( i = 0; i < numberOfPoints; i++) curve[i] = PointOnCubicBezier( cp,
	 * i*dt ); }
	 */
	 ...
	 * 
	 * cp is a 4 element array where: cp[0] is the starting point, or P0 in the
	 * above diagram cp[1] is the first control point, or P1 in the above
	 * diagram cp[2] is the second control point, or P2 in the above diagram
	 * cp[3] is the end point, or P3 in the above diagram t is the parameter
	 * value, 0 <= t <= 1
	 */
