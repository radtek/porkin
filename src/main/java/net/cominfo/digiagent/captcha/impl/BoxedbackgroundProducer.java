...
public class BoxedbackgroundProducer implements IBackgroundProducer {
	...
	public BufferedImage addBackground(BufferedImage image) {
		...
		int bacgroundRGB = Integer.parseInt(props.getProperty("background",
				"E3F1FD"), 16);
		int maxBoxes = Integer.parseInt(props.getProperty("maxboxes", "4"));
		int minBoxes = Integer.parseInt(props.getProperty("minboxes", "2"));
		boolean canIntersect = Boolean.parseBoolean(props.getProperty(
				"intersect", "false"));
		boolean border = Boolean.parseBoolean(props.getProperty(
				"border-visible", "true"));
		int borderColor = Integer.parseInt(props.getProperty("border-color",
				"000000"), 16);
		...
		for (int i = 0; i < randNumberOfBoxes; i++)
			rectangler[i] = new Rectangle();
